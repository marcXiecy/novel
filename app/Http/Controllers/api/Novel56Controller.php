...
class Novel56Controller extends Controller
{
    public $siteUrl = "https://www.56shuku.la";
    ...
    public function search(Request $request)
    {
        ...
        $htmlObj->load($books);
        // echo $htmlObj;die;
        $tr = $htmlObj->find('#articlelist li');
        $result = [];
        ...
        foreach ($tr as $k => $ele) {
            ...
            $temp['title'] = $ele->find('span a', 0)->plaintext;
            $temp['href'] = $this->siteUrl . $ele->find('span a', 0)->href;
            $result[] = $temp;
        }
        if (!$result) {
    ...
    public function catalog(Request $request)
    {
        ...
        $title = $htmlObj->find('.infot h1', 0);
        $title = $title->plaintext;
        $author = $htmlObj->find('.infot span', 0);
        ...
        foreach ($list as $ele) {
            ...
            $temp['href'] =  $catalog_url . $ele->href;
            $result['catalog'][] = $temp;
        }
    ...
    public function article(Request $request)
    {
        ...
        $result['preview'] = $this->siteUrl . $baseurl->href . substr($preview->href,2);
        $result['next'] = $this->siteUrl . $baseurl->href . substr($next->href,2);
